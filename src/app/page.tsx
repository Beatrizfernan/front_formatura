...
import { AlertCircle, Download, MapPin, Users, Calendar, Plus, Upload, X, Map } from "lucide-react"
...
export default function Home() {
  ...
  const handleFileSelect = (file: File | null) => {
    if (!file) { setArquivoSelecionado(null); return }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!arquivoSelecionado) { setError("Por favor, selecione um arquivo"); return }
    if (!localId) { setError("Por favor, selecione um local"); return }
  ...
  // Abre o mapa interativo em nova aba
  const handleVerMapa = () => {
    if (!result?.formatura?.id) return
    window.open(`/mapa/${result.formatura.id}`, "_blank")
  }

  // Baixa o PDF do backend (lê sempre a versão mais recente do banco)
  const handleDownload = async () => {
    if (!result?.formatura?.id) { setError("Dados da formatura não disponíveis"); return }
    setLoadingPdf(true)
    try {
      const response = await fetch(`${API_URL}/api/pdf/mapa-assentos/${result.formatura.id}`, {
      ...
      <div className="mx-auto max-w-7xl space-y-8">
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex-1 space-y-2">
            <h1 className="text-4xl font-bold tracking-tight">Sistema de Alocação de Assentos</h1>
            <p className="text-muted-foreground text-lg">Gerencie a distribuição de assentos para formaturas</p>
          </div>
          <Button variant="outline" size="lg" onClick={() => window.location.href = "/criar-local"} className="shrink-0">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Faça upload da planilha e selecione o local para gerar a alocação</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                >
                  ...
                  <div className="flex flex-col items-center justify-center gap-3">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-sm text-muted-foreground mt-1">Aceita CSV ou Excel (máximo 5MB)</p>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-700">{(arquivoSelecionado.size / 1024).toFixed(2)} KB</p>
                      </div>
                    </div>
                    <Button type="button" variant="ghost" size="icon" onClick={() => setArquivoSelecionado(null)} className="h-8 w-8">
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                )}
                  ...
                  <SelectContent>
                    {locais.map((local) => (
                      <SelectItem key={local.id} value={local.id}>
                        {local.nome}{local.capacidade && ` (${local.capacidade} lugares)`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <Button type="submit" className="w-full" disabled={loading || !localId || !arquivoSelecionado} size="lg">
                {loading ? (
        ...
        {result && (
          <div className="space-y-6">
            ...
            {/* Card de ações — sem o mapa inline */}
            {result.alocacao && (
              <Card className="shadow-lg">
                <CardHeader>
                  <CardTitle>Mapa de Assentos</CardTitle>
                  <CardDescription>
                    Visualize e edite a distribuição de assentos, ou exporte o PDF final
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-col sm:flex-row gap-3">
                    {/* Botão principal: abre o mapa interativo em nova aba */}
                    <Button
                      className="flex-1"
                      size="lg"
                    ...
                    {/* Exportar PDF — sempre lê a versão mais recente do banco */}
                    <Button
                      variant="outline"
                      size="lg"
                    ...
                    >
                      {loadingPdf ? (
                        <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-foreground mr-2" />Gerando PDF...</>
                      ) : (
                  ...
                  <p className="text-xs text-muted-foreground mt-3 text-center">
                    O PDF exportado sempre reflete as últimas alterações feitas no mapa
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            {!result.alocacao && (
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>Nenhuma alocação foi gerada para esta formatura.</AlertDescription>
              </Alert>
            )}
          </div>
        )}
